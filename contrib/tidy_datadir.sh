...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Community database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Community datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Community datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Community 0.7 datadir."
    ;;
  3)
    echo "Detected Community pre-0.8 datadir."
    ;;
  4)
    echo "Detected Community 0.8 datadir."
    ;;
esac
