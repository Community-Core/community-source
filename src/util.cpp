...
const char * const BITCOIN_CONF_FILENAME = "community.conf";
const char * const BITCOIN_PID_FILENAME = "communityd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "community" is a composite category enabling all Community-related debug output
                if(ptrCategory->count(std::string("community"))) {
...
#else
    const char* pszModule = "community";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Community
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Community
    // Mac: ~/Library/Application Support/Community
    // Unix: ~/.community
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Community";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Community";
#else
    // Unix
    return pathRet / ".community";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty community.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override community.conf
